...
import com.example.clinometer.main.map.NavigationMapCleanup
...
import com.example.clinometer.main.location.FirstLocationInitializer
import com.example.clinometer.main.location.LocationPermissionHelper
...
import com.example.clinometer.main.session.SessionRaceBuilder
import com.example.clinometer.main.session.SessionSaveCoordinator
...
import com.example.clinometer.main.ui.OrientationLockBinder
...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private val serviceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            foregroundService?.getLastLocation()?.let { location ->
                ...
                val locationAgeMs = if (location.time > 0L) kotlin.math.abs(wallNow - location.time) else Long.MAX_VALUE
                val canBootstrapFromServiceLocation = locationAgeMs <= 1200L

                if (canBootstrapFromServiceLocation) {
                    ...
                    val centerLat = if (isNorthUpMode) geoPoint.latitude else geoPoint.latitude + offsetLat
                    val centerLon = if (isNorthUpMode) geoPoint.longitude else geoPoint.longitude + offsetLon

                    mapboxMapView?.mapboxMap?.setCamera(
                        CameraOptions.Builder()
                            .center(MapboxPoint.fromLngLat(centerLon, centerLat))
                            .zoom(currentZoom.toDouble())
                            .bearing(if (isNorthUpMode) 0.0 else (-location.bearing).toDouble())
                            .pitch(getCameraPitch())
                            .build()
                    )
                    suppressMapCameraUpdatesUntil = SystemClock.elapsedRealtime() + 700L
    ...
    private fun setupButtons() {
        MainControlsBinder.bindSessionControls(
            ...
            onReset = {
                if (LocationPermissionHelper.ensure(this, 1000)) {
    ...
    private fun initializeFirstLocation(location: Location) {
        if (isFirstLocation) {
            ...
            val setup = FirstLocationInitializer.compute(
                location = location,
                density = resources.displayMetrics.density,
                zoomLevel = currentZoom
            )

                val startupCenterLat = if (isNorthUpMode) setup.geoPoint.latitude else setup.centerLat
                val startupCenterLon = if (isNorthUpMode) setup.geoPoint.longitude else setup.centerLon

                mapboxMapView?.mapboxMap?.setCamera(
                CameraOptions.Builder()
                    .center(MapboxPoint.fromLngLat(startupCenterLon, startupCenterLat))
                    .zoom(currentZoom.toDouble())
                    .bearing(if (isNorthUpMode) 0.0 else (-location.bearing).toDouble())
                    .pitch(getCameraPitch())
                    .build()
            ...
            motionPredictor.addSample(setup.geoPoint, location.bearing, location.speed)

            distanceTracker.seedIfNeeded(setup.geoPoint)
    ...
    private fun saveAndFinishSession() {
        try {
            ...
            val race = SessionRaceBuilder.build(
                context = this,
                profile = currentProfile,
                routePoints = rawRoutePoints,
                serviceDuration = foregroundService?.getServiceDuration() ?: 0,
                maxSpeed = foregroundService?.getMaxSpeed() ?: 0f,
                totalDistanceKm = distanceTracker.totalDistanceKm,
                maxLeftAngle = maxLeftAngle,
                maxRightAngle = maxRightAngle
            )


            SessionSaveCoordinator.persistRace(this, race, rawRoutePoints)
    ...
    private fun updateMapboxMapAnimation() {
        ...
        if (nowRealtime < suppressMapCameraUpdatesUntil) {
            mapboxTargetPosition?.let { latestTarget ->
                mapboxCurrentPosition = latestTarget
            }
        ...
        val motion = if (isNorthUpMode) {
            com.example.clinometer.main.location.MapMotionSmoothingResult(
                position = targetPos,
                bearing = targetBearing
            )
        } else {
            MapMotionSmoother.compute(
        ...
        val point = if (isNorthUpMode) {
            MapboxPoint.fromLngLat(currentPosition.longitude, currentPosition.latitude)
        } else {
            MapboxPoint.fromLngLat(cameraCenter.longitude, cameraCenter.latitude)
        }
    ...
    private fun onDestinationReached() {
        ...
        if (mapboxMapView != null) {
            mapboxMapView?.mapboxMap?.getStyle { style ->
                try {
                    NavigationMapCleanup.clearNavigationLayers(style)
                } catch (e: Exception) {
                }
            }
        }
