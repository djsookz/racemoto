...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private fun setupSensors() {
        ...
        // Lean трябва да се държи еднакво преди/след START.
        // Затова gyro + rotation работят и в staging режима.
    ...
    private fun startSession() {
        ...
        // НЕ reset-ваме lean fusion state тук,
        // за да няма скок/смяна в поведението при натискане на START.
        offsetAngle = profileLeanOffsetDeg + runtimeLeanOffsetDeg
        ...
        event?.let { ev ->
            ...
            when (ev.sensor.type) {
                ...
                Sensor.TYPE_GYROSCOPE -> {
                    ...
                    gyroscopeData.add(ev.values[0])
                    gyroscopeData.add(ev.values[1])
                    gyroscopeData.add(ev.values[2])
                    if (gyroscopeData.size > 1200) {
                        gyroscopeData.subList(0, gyroscopeData.size - 1000).clear()
                    }
                    // Add to current lap data
                    if (isRecording && lapStartTime > 0L) {
                        currentLapData.gyroscopeData.add(ev.values[0])
                        currentLapData.gyroscopeData.add(ev.values[1])
                        currentLapData.gyroscopeData.add(ev.values[2])
    ...
    private fun processLinearAccelerationAndUpdate(deviceAccel: FloatArray, timestampNs: Long) {
        ...
        if (isMotorcycle) {
            // onSensorChanged върви на main thread, директният ъпдейт избягва queue натоварване.
        ...
        // Add to current lap data
        if (isRecording && lapStartTime > 0L) {
            currentLapData.accelerationData.add(deviceAccel[0])
            currentLapData.accelerationData.add(deviceAccel[1])
            currentLapData.accelerationData.add(deviceAccel[2])
    ...
    private fun beginTimedSession(location: Location) {
        ...
        resetPeakDetectors()
        smoothedConfidence = 1f
        // Запазваме lean fusion state и при crossing на старт линията,
        // за да остане еднакво преди/след реалния старт на сесията.
        offsetAngle = profileLeanOffsetDeg + runtimeLeanOffsetDeg
    ...
    override fun onResume() {
        ...
        // ACCELEROMETER вече е регистриран в setupSensors() (винаги активен за g-сили)
        linearAccelSensor?.let { sensorManager.registerListener(this, it, SensorManager.SENSOR_DELAY_GAME) }
        // Поддържаме еднакъв lean pipeline независимо дали сесията е стартирана.
        rotationVector?.let { sensorManager.registerListener(this, it, SensorManager.SENSOR_DELAY_GAME) }
        gyroscope?.let { sensorManager.registerListener(this, it, SensorManager.SENSOR_DELAY_UI) }
    }
    ...
    private fun saveLapData(editor: android.content.SharedPreferences.Editor, sessionId: String, outingNumber: Int) {
        ...
        // Apply smart map matching to each lap (data is already finalized in lapData)
        val processedLapData = lapData
