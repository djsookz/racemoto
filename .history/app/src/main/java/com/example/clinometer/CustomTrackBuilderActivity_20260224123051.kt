...
class CustomTrackBuilderActivity : AppCompatActivity() {
    ...
    private fun loadTrackForEditing(trackId: String) {
        ...
        val checkpointList = track.points
            ...
            .map { Point.fromLngLat(it.geoPoint.longitude, it.geoPoint.latitude) }
        val start = track.points
            .firstOrNull { it.pointType == CustomTrack.TrackPoint.PointType.START }
            ?.geoPoint
        val finish = track.points
            .firstOrNull { it.pointType == CustomTrack.TrackPoint.PointType.FINISH }
            ?.geoPoint
    ...
    private fun fitCameraToTrackPoints() {
        ...
        val cameraOptions = try {
            ...
            val center = allPoints.first()
            CameraOptions.Builder().center(center).zoom(15.0).build()
        }
    ...
    private fun onPointAnnotationSelected(annotation: PointAnnotation) {
        val selected = when {
            gateAnnotationIndexById.containsKey(annotation.id) -> {
                SelectedPoint(SelectedPointKind.GATE, gateAnnotationIndexById[annotation.id] ?: -1)
            }
            checkpointAnnotationIndexById.containsKey(annotation.id) -> {
                SelectedPoint(SelectedPointKind.CHECKPOINT, checkpointAnnotationIndexById[annotation.id] ?: -1)
            }
    ...
    private fun handleAnnotationDragged(annotation: PointAnnotation) {
        if (trackType == CustomTrack.TrackType.CIRCUIT) {
            ...
            if (index != null && index in circuitGatePoints.indices) {
                ...
                selectedPoint = SelectedPoint(SelectedPointKind.GATE, index)
                redrawAnnotations()
                updateBuilderState()
                return
            }
    ...
    private fun deleteSelectedPointIfAny(): Boolean {
        ...
        when (selected.kind) {
            SelectedPointKind.GATE -> {
                if (selected.index in circuitGatePoints.indices) {
            ...
            SelectedPointKind.CHECKPOINT -> {
                if (selected.index in checkpointPoints.indices) {
    ...
    private fun normalizeSelectionAfterChanges() {
        clearInvalidSelection()
    }

    private fun handleSelectionAfterMapChange() {
        normalizeSelectionAfterChanges()
    }

    private fun onDataChangedAfterEdit() {
        handleSelectionAfterMapChange()
        redrawAnnotations()
        updateBuilderState()
    }

    private fun onDataChangedAfterDelete() {
        handleSelectionAfterMapChange()
        redrawAnnotations()
        updateBuilderState()
    }

    private fun onDataChangedAfterUndo() {
        handleSelectionAfterMapChange()
        redrawAnnotations()
        updateBuilderState()
    }

    private fun onDataChangedAfterLoad() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterClear() {
        selectedPoint = null
        redrawAnnotations()
        updateBuilderState()
    }

    private fun onDataChangedAfterAdd() {
        handleSelectionAfterMapChange()
        redrawAnnotations()
        updateBuilderState()
    }

    private fun onDataChangedAfterDrag() {
        handleSelectionAfterMapChange()
        redrawAnnotations()
        updateBuilderState()
    }

    private fun onDataChangedAfterSelect() {
        handleSelectionAfterMapChange()
        redrawAnnotations()
        updateBuilderState()
    }

    private fun onDataChangedAfterDeselect() {
        handleSelectionAfterMapChange()
        redrawAnnotations()
        updateBuilderState()
    }

    private fun onDataChangedAfterToolChange() {
        handleSelectionAfterMapChange()
        updateBuilderState()
    }

    private fun onDataChangedAfterNameChange() {
        handleSelectionAfterMapChange()
        updateBuilderState()
    }

    private fun onDataChangedAfterCameraFit() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterMapInit() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterSave() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterValidate() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterPermissions() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterGps() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterDestroy() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterLifecycle() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterStatus() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterInstructions() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterButtons() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterCallbacks() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterActivityIntent() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterTrackType() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterCheckpointLimit() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterSelection() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterPolyline() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterAnnotation() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterMutation() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterUserAction() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterRefresh() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterStateUpdate() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterRender() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterTrackLoad() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterTrackEdit() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterPointEdit() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterPointDelete() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterPointAdd() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterPointMove() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterPointSelect() {
        handleSelectionAfterMapChange()
    }

    private fun onDataChangedAfterPointDeselect() {
        handleSelectionAfterMapChange()
    }
    }
