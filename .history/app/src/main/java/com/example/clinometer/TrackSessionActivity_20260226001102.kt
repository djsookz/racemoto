...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private val statsDeadbandG = 0.12f
    ...
    private fun updateSessionGForceStatistics(rawLatG: Float, rawLongG: Float) {
        ...
        maxAcceleration = max(maxAcceleration, max(0f, longitudinal))
        maxBraking = max(maxBraking, max(0f, -longitudinal))
        maxCorneringLeftG = max(maxCorneringLeftG, max(0f, lateral))
        maxCorneringRightG = max(maxCorneringRightG, max(0f, -lateral))
    }

    private fun updateInertialForcesFromRawAccelerometer(rawAccel: FloatArray) {
        val isCalibrated = DragCalibration.isUniversalCalibrated
        val liveGravity = gravity.clone()

        val rawGForceX: Float
        val rawGForceY: Float

        if (isCalibrated) {
            val forwardAccel = DragCalibration.getSignedForwardAcceleration(rawAccel, liveGravity)
            val lateralAccel = DragCalibration.getSignedLateralAcceleration(rawAccel, liveGravity)

            rawGForceX = -lateralAccel / 9.81f
            rawGForceY = -forwardAccel / 9.81f
        } else {
            rawGForceX = (rawAccel[0] - liveGravity[0]) / 9.81f
            rawGForceY = (rawAccel[1] - liveGravity[1]) / 9.81f
        }

        val speedMs = lastLocation?.speed ?: 0f
        val canLearnBias = isStationary || !isRecording || awaitingStart || speedMs < 0.8f
        if (canLearnBias) {
            forwardBiasG = (1f - biasAlpha) * forwardBiasG + biasAlpha * rawGForceY
            lateralBiasG = (1f - biasAlpha) * lateralBiasG + biasAlpha * rawGForceX
        }

        var correctedLongG = rawGForceY - forwardBiasG
        var correctedLatG = rawGForceX - lateralBiasG

        if (abs(correctedLongG) < deadbandG) correctedLongG = 0f
        if (abs(correctedLatG) < deadbandG) correctedLatG = 0f

        displayLY = displaySmoothAlpha * correctedLongG + (1f - displaySmoothAlpha) * displayLY
        displayLX = displaySmoothAlpha * correctedLatG + (1f - displaySmoothAlpha) * displayLX

        speedGauge.gForceX = displayLX
        speedGauge.gForceY = displayLY
        updateSessionGForceStatistics(displayLX, displayLY)
    }
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        event?.let { ev ->
            ...
            if (ev.sensor.type == Sensor.TYPE_ACCELEROMETER) {
                ...
                updateInertialForcesFromRawAccelerometer(latestRawAccel)
            }
    ...
    private fun processLinearAccelerationAndUpdate(deviceAccel: FloatArray) {
        ...
        // === Unified inertial-force pipeline (visual + max statistics) ===
        // Compute vehicle-frame acceleration using calibration first; fallback to heading projection.
        var forwardAccelMs2: Float
        var lateralAccelMs2: Float
        if (DragCalibration.isUniversalCalibrated) {
            val fwd = DragCalibration.forwardVector
            val right = DragCalibration.rightVector
            forwardAccelMs2 = deviceAccel[0] * fwd[0] + deviceAccel[1] * fwd[1] + deviceAccel[2] * fwd[2]
            lateralAccelMs2 = deviceAccel[0] * right[0] + deviceAccel[1] * right[1] + deviceAccel[2] * right[2]
        } else {
            ...
            forwardAccelMs2 = north * cosB + east * sinB
            lateralAccelMs2 = east * cosB - north * sinB
        }

        // Convert acceleration to inertial force (opposite direction of acceleration).
        val rawLongG = -forwardAccelMs2 / 9.81f
        val rawLatG = -lateralAccelMs2 / 9.81f

        // Adaptive zero-bias update while stationary/idle to avoid boomerang drift.
        val speedMs = lastLocation?.speed ?: 0f
        val canLearnBias = isStationary || !isRecording || awaitingStart || speedMs < 0.8f
        if (canLearnBias) {
            forwardBiasG = (1f - biasAlpha) * forwardBiasG + biasAlpha * rawLongG
            lateralBiasG = (1f - biasAlpha) * lateralBiasG + biasAlpha * rawLatG
        }

        var correctedLongG = rawLongG - forwardBiasG
        var correctedLatG = rawLatG - lateralBiasG
