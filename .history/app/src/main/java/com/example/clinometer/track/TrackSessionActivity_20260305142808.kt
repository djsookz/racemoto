...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private lateinit var tvPredictiveGapSign: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val intentIsMotorcycle = intent.getBooleanExtra("is_motorcycle", profileIsMotorcycle)
        if (intent.hasExtra("is_motorcycle") && intentIsMotorcycle != profileIsMotorcycle) {
            Log.w(
                "TrackSessionActivity",
                "Ignoring mismatched is_motorcycle extra ($intentIsMotorcycle); using profile mode ($profileIsMotorcycle)"
            )
        }
        isMotorcycle = profileIsMotorcycle
    ...
    private fun initializeViews() {
        ...
        tvPredictiveGapSign = findViewById(R.id.tvPredictiveGapSign)
        ...
        topTelemetryRow.visibility = if (isMotorcycle) View.VISIBLE else View.GONE
        cardPredictiveLap.visibility = if (isMotorcycle) View.VISIBLE else View.GONE
    ...
    private fun updateTopLeanTelemetry(leanAngle: Float = currentCalibratedLean) {
        ...
        leanVisualizer.setLeanAngle(leanAngle)
        when {
            absLean < 2f -> {
        ...
        val progress01 = forcedProgress?.coerceIn(0f, 1f) ?: run {
            if (!isRecording || awaitingStart || lapStartTime <= 0L) {
                ...
                if (targetDistance <= 0f || lapDistanceAccum <= 0f) {
                    ...
                    val remainingByTarget = (targetDistance - lapDistanceAccum).coerceAtLeast(0f)
                    val lineRemaining = currentDistanceToFinishLineMeters
                    val effectiveRemaining = if (lineRemaining.isFinite() && lineRemaining >= 0f) {
                        max(remainingByTarget, lineRemaining)
                    } else {
                        remainingByTarget
                    }

                    val denominator = lapDistanceAccum + effectiveRemaining
                    if (denominator <= 0.1f) {
                        0f
                    } else {
                        (lapDistanceAccum / denominator).coerceIn(0f, 0.998f)
                    }
                }
            }
        }
    ...
    private fun trackBestReferenceText(): String {
        return if (trackBestLapNumber > 0 && trackBestLapTime != Long.MAX_VALUE) {
            "LAP $trackBestLapNumber - ${formatLapTimePrecise(trackBestLapTime)}"
        } else {
    ...
    private fun resetPredictiveGapCard() {
        tvPredictiveGapSign.text = "+"
        tvPredictiveGapSignValue.text = "+"
        tvPredictiveGapValue.text = "0.0"
        val baseColor = ContextCompat.getColor(this, R.color.accent_green)
        tvPredictiveGapSign.setTextColor(baseColor)
    ...
    private fun updatePredictiveGapCard(predictedLapSeconds: Float, referenceLapSeconds: Float) {
        ...
        val sign = if (isPositiveGap) "+" else "-"
        val displayValue = kotlin.math.abs(gapSeconds)
        val color = ContextCompat.getColor(this, if (isPositiveGap) R.color.accent_red else R.color.accent_green)

        tvPredictiveGapSign.text = sign
        tvPredictiveGapSignValue.text = sign
        tvPredictiveGapValue.text = String.format(Locale.US, "%.1f", displayValue)
        tvPredictiveGapSign.setTextColor(color)
        ...
        for (sessionId in sessionIds) {
            ...
            for (outing in 1..outingCount) {
                ...
                for (lap in 1..lapDataCount) {
                    ...
                    val lapMs = parseLapTimeToMillis(lapText)
                    if (lapMs in 1 until bestTime) {
                        bestTime = lapMs
                        bestLap = lap
                    }
                }
    ...
    private fun updateGauge() {
        ...
        if (isRecording && !awaitingStart && lapStartTime > 0 && bestLapTime != Long.MAX_VALUE && bestLapDistance > 0f) {
            ...
            val referenceLapSeconds = when (predictiveGapSource) {
                PredictiveGapSource.SESSION_BEST -> bestLapTime / 1000f
                PredictiveGapSource.TRACK_BEST -> {
                    if (trackBestLapTime != Long.MAX_VALUE) trackBestLapTime / 1000f else bestLapTime / 1000f
                }
            }
