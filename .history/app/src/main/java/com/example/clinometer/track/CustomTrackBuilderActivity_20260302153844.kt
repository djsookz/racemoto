...
class CustomTrackBuilderActivity : AppCompatActivity() {
    ...
    private fun setupButtons() {
        ...
        btnImportTrack.setOnClickListener {
            importTrackLauncher.launch(arrayOf("application/json", "text/plain"))
        }
    ...
    private fun importTrackFromUri(uri: android.net.Uri) {
        try {
            ...
            if (content.isNullOrBlank()) {
                Toast.makeText(this, "Празен или невалиден файл", Toast.LENGTH_SHORT).show()
                return
            }

            val imported = gson.fromJson(content, CustomTrackExchange::class.java)
            val importedOk = applyImportedTrack(imported)
            if (importedOk) {
                Toast.makeText(this, "Пистата е импортната успешно", Toast.LENGTH_SHORT).show()
            }
        } catch (e: Exception) {
            Toast.makeText(this, "Неуспешен импорт: ${e.message}", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun exportTrackToUri(uri: android.net.Uri) {
        try {
            ...
            Toast.makeText(this, "Файлът е експортиран", Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
            Toast.makeText(this, "Неуспешен експорт: ${e.message}", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun applyImportedTrack(imported: CustomTrackExchange): Boolean {
        val importedType = try {
            CustomTrack.TrackType.valueOf(imported.mode)
        } catch (_: Exception) {
            trackType
        }

        if (importedType != trackType) {
            val importedModeLabel = trackTypeLabel(importedType)
            val currentModeLabel = trackTypeLabel(trackType)
            Toast.makeText(
                this,
                "Файлът е за $importedModeLabel, а текущият трак е $currentModeLabel. Смени режима и импортни отново.",
                Toast.LENGTH_LONG
            ).show()
            return false
        }
        ...
        if (trackType == CustomTrack.TrackType.CIRCUIT) {
            circuitGatePoints.addAll(imported.circuitGatePoints.take(2).map { Point.fromLngLat(it.lon, it.lat) })
            activeTool = if (circuitGatePoints.size < 2) BuilderTool.SET_CIRCUIT_GATE else BuilderTool.SET_CHECKPOINT
        } else {
            startGatePoints.addAll(imported.startGatePoints.take(2).map { Point.fromLngLat(it.lon, it.lat) })
            finishGatePoints.addAll(imported.finishGatePoints.take(2).map { Point.fromLngLat(it.lon, it.lat) })
            activeTool = when {
    ...
    private fun trackTypeLabel(type: CustomTrack.TrackType): String {
        return when (type) {
            CustomTrack.TrackType.CIRCUIT -> "обиколки"
            CustomTrack.TrackType.POINT_TO_POINT -> "точка до точка"
        }
    }
