...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private fun startRecording() {
        verifyCalibrationAndStartCountdown()
    }

    private fun verifyCalibrationAndStartCountdown() {
        ...
        when (qualityReport.level) {
            CalibrationQualityLevel.NOT_CALIBRATED -> {
                androidx.appcompat.app.AlertDialog.Builder(this)
                    ...
                    .setNeutralButton("Continue Anyway") { _, _ -> startCountdown() }
                    .setNegativeButton(getString(R.string.dialog_cancel_button), null)
                    .show()
            }
            CalibrationQualityLevel.BAD -> {
                androidx.appcompat.app.AlertDialog.Builder(this)
                    .setTitle("Low Calibration Quality")
                    .setMessage(
                        "Calibration quality is LOW. " +
                            "Braking/accel G peaks may be unstable. Recalibration is strongly recommended."
                    )
                    .setPositiveButton("Recalibrate") { _, _ -> openCalibrationScreen() }
                    .setNeutralButton("Continue Anyway") { _, _ -> startCountdown() }
                    .setNegativeButton(getString(R.string.dialog_cancel_button), null)
                    .show()
            }
            CalibrationQualityLevel.WARNING -> {
                showToast("Calibration quality WARNING.")
                startCountdown()
            }
    ...
    private fun processLinearAccelerationAndUpdate(deviceAccel: FloatArray, timestampNs: Long) {
        ...
        // Keep lean telemetry live regardless of vehicle mode so it cannot get stuck at 0
        // when profile/intent motorcycle flags are mismatched.
