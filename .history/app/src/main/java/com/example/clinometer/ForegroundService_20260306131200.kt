...
class ForegroundService : Service(), SensorEventListener {
    ...
    fun getAccuracyMode(): String = DragCalibration.getAccuracyModeByCalibrationQuality()
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        if (event.sensor.type == Sensor.TYPE_ACCELEROMETER) {
            ...
            if (DragCalibration.hasCalibrationFor(isLandscape)) {
                ...
                val lateralAccel = DragCalibration.getSignedLateralAcceleration(rawAccel, isLandscape)
            ...
            } else if (DragCalibration.isUniversalCalibrated) {
                val rawAccel = floatArrayOf(event.values[0], event.values[1], event.values[2])
    ...
    private fun checkLinearAccelStart(rawAccel: FloatArray) {
        ...
        val thresholdMultiplier = when (qualityReport.level) {
            CalibrationQualityLevel.BAD -> 1.25f
            CalibrationQualityLevel.WARNING -> 1.10f
            else -> 1.0f
        }
        val threshold = (
            DragCalibration.getWeightedDynamicThreshold(
                DragCalibration.getLinearAcceleration(rawAccel, isLandscape),
                isLandscape
            ).coerceAtLeast(0.8f) * thresholdMultiplier
        )
