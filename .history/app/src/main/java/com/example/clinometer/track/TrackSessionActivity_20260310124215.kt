...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private var hasLoggedAccelOnlyLeanFallback: Boolean = false
    ...
    private val leanPreviewDisplayDeadbandDeg: Float = 0.7f
    private val leanPreviewDisplaySmoothingAlpha: Float = 0.36f
    private val leanDisplaySnapToZeroDeg: Float = 0.25f
    private val gyroLeanSensorDelay: Int = SensorManager.SENSOR_DELAY_GAME
    ...
    private val alphaGravity = 0.98f  // Бавен low-pass за gravity isolation (fallback когато Madgwick не е инициализиран)
    // Madgwick AHRS — дава точен gravity вектор дори при динамично движение
    private val madgwick = MadgwickAHRS(beta = 0.08f)
    private val latestGyroForMadgwick = FloatArray(3)
    private var lastMadgwickUpdateNs = 0L
    ...
    private val linearAccelFreshTimeoutNs: Long = 120_000_000L
    private val gyroSampleFreshTimeoutNs: Long = 250_000_000L
    ...
    private val accelOnlyLeanAlphaRecording: Float = 0.32f
    private val accelOnlyLeanAlphaPreview: Float = 0.55f
    ...
    private fun updateTopLeanTelemetry(leanAngle: Float = currentCalibratedLean) {
        ...
        // Track moto test mode: lean telemetry is trusted only with DragCalibration vectors.
        if (!DragCalibration.isUniversalCalibrated) {
            displayLeanAngle = 0f
            hasDisplayLeanAngle = false
            tvTopLeanValue.text = "--"
            leanVisualizer.setLeanAngle(0f)
            tvTopLeanDirection.visibility = View.VISIBLE
            tvTopLeanDirection.text = "CALIBRATE"
            return
        }

        val displayDeadband = if (!isRecording && isMotorcycle) {
            leanPreviewDisplayDeadbandDeg
        } else {
            leanDisplayDeadbandDeg
        }
        val displayAlpha = if (!isRecording && isMotorcycle) {
            leanPreviewDisplaySmoothingAlpha
        } else {
            leanDisplaySmoothingAlpha
        }

        val targetLean = if (abs(leanAngle) < displayDeadband) 0f else leanAngle
        if (!hasDisplayLeanAngle) {
            displayLeanAngle = targetLean
            hasDisplayLeanAngle = true
        } else {
            displayLeanAngle += displayAlpha * (targetLean - displayLeanAngle)
        }
    ...
    private fun updateProfileLeanOffsetForOrientation(isLandscape: Boolean) {
        if (isMotorcycle && DragCalibration.isUniversalCalibrated) {
            // With vector calibration, keep only runtime zero and ignore legacy profile offsets.
            hasProfileLeanOffset = false
    ...
    private fun setupSensors() {
        ...
        if (gyroscope == null) {
            Log.w("TrackSession", "Gyroscope not available; using accelerometer-only lean fallback")
        }
        Log.i(
            "TrackSession",
            "Sensor capability accel=${accelerometer != null}, linear=${linearAccelSensor != null}, rotationVector=${rotationVector != null}, gyro=${gyroscope != null}"
        )
        
        // Регистрираме основните сензори винаги, за да работят G/lean телеметриите
        // и когато не записваме сесия.
        ...
        gyroscope?.let { sensorManager.registerListener(this, it, gyroLeanSensorDelay) }
    }
    ...
    private fun startSession() {
        ...
        gyroTimestampNs = 0L
        hasLoggedAccelOnlyLeanFallback = false
        filteredAngle = 0f
        offsetAngle = profileLeanOffsetDeg + runtimeLeanOffsetDeg
    ...
    private fun stopRecording() {
        ...
        // Оставяме inertial сензорите активни, за да продължи lean/g-force телеметрията извън запис.
    ...
    private fun updateInertialForcesFromLinearAcceleration(deviceLinearAccel: FloatArray) {
        ...
        // Учим bias само когато стоим — НЕ докато шофираме (дори без запис).
        // При !isRecording старата логика ядеше продължителни G-сили и връщаше точката в центъра.
        val canLearnBias = isStationary || awaitingStart || speedMs < 0.5f
        if (canLearnBias) {
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        event?.let { ev ->
            ...
            if (ev.sensor.type == Sensor.TYPE_ACCELEROMETER) {
                // Madgwick AHRS gravity estimate (по-точно от low-pass при динамично движение)
                ...
                madgwick.update(
                    latestGyroForMadgwick[0], latestGyroForMadgwick[1], latestGyroForMadgwick[2],
                ...
                if (madgwick.isInitialized) {
                    ...
                    gravity[0] = mg[0]; gravity[1] = mg[1]; gravity[2] = mg[2]
                } else {
                    // Fallback low-pass до инициализацията (~0.5s)
            ...
            when (ev.sensor.type) {
                ...
                Sensor.TYPE_LINEAR_ACCELERATION -> {
                    if (preferLinearAccel) {
                        linearAccelTimestampNs = ev.timestamp
                ...
                Sensor.TYPE_ACCELEROMETER -> {
                    // For low-end devices, prefer immediate accel fallback in Track preview
                    // when linear-accel samples are missing/stale.
                    val linearIsFresh = preferLinearAccel &&
                        linearAccelTimestampNs > 0L &&
                        (ev.timestamp - linearAccelTimestampNs) in 0L..linearAccelFreshTimeoutNs
                    val shouldUseAccelFallback = !preferLinearAccel || !linearIsFresh || (!isRecording && isMotorcycle)

                    if (shouldUseAccelFallback) {
                ...
                Sensor.TYPE_GYROSCOPE -> {
                    // Запазваме последния gyro за Madgwick update (ran от TYPE_ACCELEROMETER handler)
                    latestGyroForMadgwick[0] = ev.values[0]
                    latestGyroForMadgwick[1] = ev.values[1]
                    latestGyroForMadgwick[2] = ev.values[2]
                    ...
                    // Използваме calibrated forwardVector за gyro roll rate (forward ос на мотора)
                    val rawRollRateRad = if (DragCalibration.isUniversalCalibrated) {
                        ...
                        // Track moto test mode: no hardcoded phone axes fallback without vector calibration.
                        0f
                    ...
                    // On some low-end devices linear/accel callbacks are sparse.
                    // Refresh lean immediately from gyro integration when not recording,
                    // so Track preview stays responsive outside a session.
                    if (!isRecording && isMotorcycle) {
    ...
    private fun processLinearAccelerationAndUpdate(deviceAccel: FloatArray, timestampNs: Long) {
        ...
        val hasVectorLeanCalibration = DragCalibration.isUniversalCalibrated
        if (isMotorcycle && !hasVectorLeanCalibration) {
            // Force neutral lean when no vector calibration exists in Track moto test mode.
            gyroIntegratedLeanDeg = 0f
            filteredAngle = 0f
            currentCalibratedLean = 0f
        }
        ...
        val accelReferenceTilt = if (totalGravity > 0f) {
            if (hasVectorLeanCalibration) {
                // Точно: проекция на gravity върху rightVector (ос дясно на мотора)
                val rv = DragCalibration.rightVector
                val dotRight = ((x*rv[0] + y*rv[1] + z*rv[2]) / totalGravity).toDouble()
                (-Math.toDegrees(Math.asin(dotRight.coerceIn(-1.0, 1.0)))).toFloat()
            } else if (isMotorcycle) {
                0f
            } else if (isLandscape) {
        ...
        val hasFreshGyroSample = gyroscope != null &&
            gyroTimestampNs > 0L &&
            (timestampNs - gyroTimestampNs) in 0L..gyroSampleFreshTimeoutNs

        if (hasFreshGyroSample) {
            ...
            if (!hasLoggedAccelOnlyLeanFallback) {
                Log.w("TrackSession", "Gyro missing/stale; switching to fast accelerometer-only lean tracking")
                hasLoggedAccelOnlyLeanFallback = true
            }
            // Fast fallback for devices where gyro events are missing or throttled.
            val accelOnlyAlpha = if (isRecording) accelOnlyLeanAlphaRecording else accelOnlyLeanAlphaPreview
            gyroIntegratedLeanDeg += accelOnlyAlpha * (accelReferenceTilt - gyroIntegratedLeanDeg)
            latestRollRateDegPerSec = 0f
        }
    ...
    override fun onResume() {
        ...
        // Сензорите са активни и извън запис, за да е стабилен lean ъгълът в Track екрана.
        linearAccelSensor?.let { sensorManager.registerListener(this, it, SensorManager.SENSOR_DELAY_GAME) }
        accelerometer?.let { sensorManager.registerListener(this, it, SensorManager.SENSOR_DELAY_GAME) }
        rotationVector?.let { sensorManager.registerListener(this, it, SensorManager.SENSOR_DELAY_GAME) }
        gyroscope?.let { sensorManager.registerListener(this, it, gyroLeanSensorDelay) }
