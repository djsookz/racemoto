...
class MainContainerActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        private const val LEGACY_INITIAL_PAGE = "INITIAL_PAGE"
        private const val LEGACY_NAV_ITEM_ID = "NAV_ITEM_ID"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Задаваме начална страница - поддържаме и новите, и legacy extras
        val initialPage = if (intent.hasExtra(EXTRA_INITIAL_PAGE) || intent.hasExtra(LEGACY_INITIAL_PAGE)) {
            intent.getIntExtra(EXTRA_INITIAL_PAGE, intent.getIntExtra(LEGACY_INITIAL_PAGE, PAGE_MAP))
        } else if (intent.hasExtra(EXTRA_NAV_ITEM_ID) || intent.hasExtra(LEGACY_NAV_ITEM_ID)) {
            val navItemId = intent.getIntExtra(EXTRA_NAV_ITEM_ID, intent.getIntExtra(LEGACY_NAV_ITEM_ID, R.id.navMap))
