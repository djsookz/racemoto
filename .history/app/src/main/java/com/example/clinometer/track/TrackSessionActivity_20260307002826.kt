    ...
    private fun updateLapSummaryCards(currentLapElapsedMs: Long? = null) {
        val currentValue = when {
            isRecording && !awaitingStart && lapStartTime > 0L -> {
                formatTime(currentLapElapsedMs ?: (System.currentTimeMillis() - lapStartTime))
            }
            else -> "00:00.00"
        }
        val bestValue = if (bestLapTime != Long.MAX_VALUE) formatTime(bestLapTime) else "--:--.--"
        val lastValue = if (lapTimes.isNotEmpty()) formatTime(lapTimes.last()) else "--:--.--"
