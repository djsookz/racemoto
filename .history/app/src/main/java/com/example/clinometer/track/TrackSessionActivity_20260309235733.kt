...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private val leanDisplaySmoothingAlpha: Float = 0.6f
    ...
    private val minAccelCorrection = 0.005f
    private val maxAccelCorrection = 0.05f
    ...
    private fun stopRecording() {
        ...
        // Оставяме inertial сензорите активни в Track екрана,
        // за да е live lean без лаг и преди/след запис.
        ...
        event?.let { ev ->
            ...
            when (ev.sensor.type) {
                ...
                Sensor.TYPE_GYROSCOPE -> {
                    ...
                    latestRollRateDegPerSec = 0.6f * rollRateDeg + 0.4f * latestRollRateDegPerSec

                    // Инициализираме интегратора веднага от gyro потока, за да не чакаме
                    // TYPE_LINEAR_ACCELERATION (което при някои устройства е бавно/с латентност).
                    if (!hasGyroIntegratedLean) {
                        gyroIntegratedLeanDeg = (currentCalibratedLean + offsetAngle).coerceIn(-89f, 89f)
                    ...
                    // Ниска латентност: показваме lean директно от gyro интегратора на всяко
                    // gyro събитие, вместо да чакаме linear-accel pipeline.
    ...
    private fun processLinearAccelerationAndUpdate(deviceAccel: FloatArray, timestampNs: Long) {
        ...
        // При активно лягане (висок lean rate) акселерометърът е ненадежден → гироскопът доминира
        val leanRatePenalty = (abs(latestRollRateDegPerSec) / 15f).coerceIn(0f, 1f)
        val accelTrust = (accelMotionTrust * (1f - leanRatePenalty)).coerceIn(0f, 1f)
