...
object DragCalibration {
    ...
    private fun loadFromPrefs() {
        ...
        prefs?.let { p ->
            ...
            // Backward compatibility - keep legacy flags coherent with universal calibration.
            if (isPortraitCalibrated) {
            ...
            } else if (isUniversalCalibrated) {
                forwardAxis = forwardVector.clone()
                lateralAxis = rightVector.clone()
                isCalibrated = true
                calibrationTime = universalCalibrationTime
            } else {
                isCalibrated = false
                calibrationTime = 0L
    ...
    fun lockUniversalCalibration(gravity: FloatArray, forward: FloatArray, maxVibration: Float) {
        ...
        // Backward compatibility: when only universal calibration exists,
        // legacy checks should still pass and use the calibrated axes.
        forwardAxis = forwardVector.clone()
        lateralAxis = rightVector.clone()
        isCalibrated = true
        calibrationTime = universalCalibrationTime
