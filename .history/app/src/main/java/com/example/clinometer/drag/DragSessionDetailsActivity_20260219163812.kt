...
class DragAttemptsAdapter(
    ...
    private fun findClosestSpecialPointByPixel(
        ...
        val tapRadiusPx = tapRadiusDp * context.resources.displayMetrics.density
        val candidatePoints = mutableListOf<Pair<SpecialPoint, Float>>()

        for (point in specialPoints) {
            val pos = chart.getPosition(
                com.github.mikephil.charting.data.Entry(point.x, point.y),
                com.github.mikephil.charting.components.YAxis.AxisDependency.LEFT
            )
            val dx = kotlin.math.abs(touchXpx - pos.x)
            val dy = kotlin.math.abs(touchYpx - pos.y)
            if (dx <= tapRadiusPx && dy <= tapRadiusPx) {
                val distance = kotlin.math.sqrt(dx * dx + dy * dy)
                candidatePoints.add(point to distance)
            }
        }
