...
class SpeedGaugeView @JvmOverloads constructor(
    ...
    private fun drawCenterDisk(canvas: Canvas) {
        ...
        // Max left and max right inside the circle, flanking the speed
        textPaint.textSize = dp(22f)
        textPaint.color = Color.parseColor("#ECEFF5")
        canvas.drawText("${maxLeanLeft.roundToInt()}°", centerX - radius * 0.58f, centerY + radius * 0.10f, textPaint)
        canvas.drawText("${maxLeanRight.roundToInt()}°", centerX + radius * 0.58f, centerY + radius * 0.10f, textPaint)
