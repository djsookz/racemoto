...
class TrackLapCompareActivity : AppCompatActivity() {
    ...
    private lateinit var btnModeSpeed: TextView
    private lateinit var btnModeAngle: TextView
    ...
    private fun bindViews() {
        ...
        btnModeSpeed = findViewById(R.id.btnModeSpeed)
        btnModeAngle = findViewById(R.id.btnModeAngle)
    ...
    private fun setupModeTabs() {
        btnModeAngle.visibility = if (isMotorcycleCompare) android.view.View.VISIBLE else android.view.View.GONE

        btnModeSpeed.setOnClickListener {
            if (compareMode != CompareMode.SPEED) {
                compareMode = CompareMode.SPEED
                applyModeTabStyles()
                applyChartModeData()
        ...
        btnModeAngle.setOnClickListener {
            if (isMotorcycleCompare && compareMode != CompareMode.ANGLE) {
                compareMode = CompareMode.ANGLE
                applyModeTabStyles()
                applyChartModeData()
                updateByTime(currentReaderTimeSec)
            }
    ...
    private fun applyModeTabStyles() {
        val activeBg = Color.parseColor("#FF6020")
        val inactiveBg = Color.parseColor("#1E2633")
        val activeText = Color.WHITE
        val inactiveText = Color.parseColor("#AEB7C6")

        btnModeSpeed.setBackgroundColor(if (compareMode == CompareMode.SPEED) activeBg else inactiveBg)
        btnModeSpeed.setTextColor(if (compareMode == CompareMode.SPEED) activeText else inactiveText)

        if (isMotorcycleCompare) {
            btnModeAngle.setBackgroundColor(if (compareMode == CompareMode.ANGLE) activeBg else inactiveBg)
            btnModeAngle.setTextColor(if (compareMode == CompareMode.ANGLE) activeText else inactiveText)
        }
    }
    ...
    private fun updateByTime(timeSec: Float) {
        ...
        when (compareMode) {
            CompareMode.SPEED -> {
                tvCurrentValue.text = UnitsManager.formatSpeed(currentPoint.speed, this, 0)
                tvCompareValue.text = UnitsManager.formatSpeed(comparePoint.speed, this, 0)
            }

            CompareMode.ANGLE -> {
                tvCurrentValue.text = String.format("%.0f°", abs(currentPoint.angle))
                tvCompareValue.text = String.format("%.0f°", abs(comparePoint.angle))
            }
        }
