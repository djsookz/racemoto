...
class SpeedGaugeView @JvmOverloads constructor(
    ...
    private fun drawMotoTelemetry(canvas: Canvas) {
        val brakingG = max(0f, gForceY)
        val accelG = max(0f, -gForceY)
        val peakBrake = peakBrakeG
        val peakAccel = peakAccelG
        val totalLongPeak = peakBrake + peakAccel
        ...
        textPaint.color = Color.parseColor("#6E88A4")
        canvas.drawText("G-FORCE - LONGITUDINAL", left, top + dp(6f), textPaint)

        textPaint.textAlign = Paint.Align.RIGHT
        textPaint.textSize = dp(8f)
        textPaint.color = Color.parseColor("#6E88A4")
        canvas.drawText("TOTAL", right - dp(34f), top + dp(6f), textPaint)

        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        textPaint.textSize = dp(22f)
        textPaint.color = Color.parseColor("#F9B028")
        canvas.drawText(String.format("%.1f", totalLongPeak), right, top + dp(8f), textPaint)

        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.NORMAL)
        textPaint.textSize = dp(9f)
        textPaint.color = Color.parseColor("#6E88A4")
        canvas.drawText("g", right + dp(8f), top + dp(8f), textPaint)

        val centerX = width / 2f
        val axisTop = top + dp(24f)
        val axisBottom = bottom - dp(16f)

        arcPaint.style = Paint.Style.STROKE
        arcPaint.strokeCap = Paint.Cap.ROUND
        arcPaint.strokeWidth = dp(1.2f)
        arcPaint.color = Color.parseColor("#415368")
        canvas.drawLine(centerX, axisTop, centerX, axisBottom, arcPaint)

        for (i in 0..4) {
            val y = axisTop + i * (axisBottom - axisTop) / 4f
            canvas.drawLine(centerX - dp(5f), y, centerX + dp(5f), y, arcPaint)
        }

        textPaint.textAlign = Paint.Align.CENTER
        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        textPaint.textSize = dp(6.5f)
        textPaint.color = Color.parseColor("#FF4E6F")
        canvas.drawText("BRAKE", centerX, axisTop - dp(4f), textPaint)
        textPaint.color = Color.parseColor("#00E985")
        canvas.drawText("ACCEL", centerX, axisBottom + dp(10f), textPaint)

        val longRangeG = 2.5f
        val clampedLong = gForceY.coerceIn(-longRangeG, longRangeG)
        val t = ((clampedLong + longRangeG) / (2f * longRangeG)).coerceIn(0f, 1f)
        val dotY = axisBottom - t * (axisBottom - axisTop)
        fillPaint.color = if (clampedLong >= 0f) Color.parseColor("#FF4E6F") else Color.parseColor("#00E985")
        canvas.drawCircle(centerX, dotY, dp(5.2f), fillPaint)

        val leftGroupLeft = left + dp(2f)
        val leftGroupRight = centerX - dp(28f)

        fillPaint.color = Color.parseColor("#59667A")
        canvas.drawRect(leftGroupLeft, top + dp(44f), leftGroupLeft + dp(8f), top + dp(52f), fillPaint)

        textPaint.textAlign = Paint.Align.LEFT
        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        textPaint.textSize = dp(10f)
        textPaint.color = Color.parseColor("#FF4E6F")
        canvas.drawText("BRAKING", leftGroupLeft + dp(18f), top + dp(51f), textPaint)

        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        textPaint.textSize = dp(34f)
        textPaint.color = Color.parseColor("#FF4E6F")
        canvas.drawText(String.format("%.1f", brakingG), leftGroupLeft + dp(86f), top + dp(74f), textPaint)
        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.NORMAL)
        textPaint.textSize = dp(11f)
        textPaint.color = Color.parseColor("#6E88A4")
        canvas.drawText("g", leftGroupLeft + dp(134f), top + dp(74f), textPaint)

        val barTop = top + dp(80f)
        val barBottom = barTop + dp(9f)
        fillPaint.color = Color.parseColor("#394758")
        canvas.drawRoundRect(RectF(leftGroupLeft, barTop, leftGroupRight, barBottom), dp(4f), dp(4f), fillPaint)
        val brakeFill = (brakingG / longRangeG).coerceIn(0f, 1f)
        fillPaint.color = Color.parseColor("#E64563")
        canvas.drawRoundRect(
            RectF(leftGroupLeft, barTop, leftGroupLeft + (leftGroupRight - leftGroupLeft) * brakeFill, barBottom),
            dp(4f),
            dp(4f),
            fillPaint
        )
        ...
        textPaint.textSize = dp(8f)
        textPaint.color = Color.parseColor("#6E88A4")
        canvas.drawText("MAX SESSION", leftGroupLeft, top + dp(104f), textPaint)
        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        textPaint.textSize = dp(14f)
        textPaint.color = Color.parseColor("#E64563")
        canvas.drawText(String.format("%.1fg", peakBrake), leftGroupRight - dp(30f), top + dp(104f), textPaint)

        val rightGroupLeft = centerX + dp(28f)
        val rightGroupRight = right - dp(2f)

        fillPaint.color = Color.parseColor("#59667A")
        canvas.drawRect(rightGroupLeft, top + dp(44f), rightGroupLeft + dp(8f), top + dp(52f), fillPaint)

        textPaint.textAlign = Paint.Align.LEFT
        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        textPaint.textSize = dp(10f)
        textPaint.color = Color.parseColor("#00E985")
        canvas.drawText("ACCEL", rightGroupLeft + dp(18f), top + dp(51f), textPaint)

        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        textPaint.textSize = dp(34f)
        textPaint.color = Color.parseColor("#00E985")
        canvas.drawText(String.format("%.1f", accelG), rightGroupLeft + dp(74f), top + dp(74f), textPaint)
        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.NORMAL)
        textPaint.textSize = dp(11f)
        textPaint.color = Color.parseColor("#6E88A4")
        canvas.drawText("g", rightGroupLeft + dp(122f), top + dp(74f), textPaint)

        fillPaint.color = Color.parseColor("#394758")
        canvas.drawRoundRect(RectF(rightGroupLeft, barTop, rightGroupRight, barBottom), dp(4f), dp(4f), fillPaint)
        val accelFill = (accelG / longRangeG).coerceIn(0f, 1f)
        fillPaint.color = Color.parseColor("#00CC75")
        canvas.drawRoundRect(
            RectF(rightGroupLeft, barTop, rightGroupLeft + (rightGroupRight - rightGroupLeft) * accelFill, barBottom),
            dp(4f),
            dp(4f),
            fillPaint
        )
        ...
        textPaint.textSize = dp(8f)
        textPaint.color = Color.parseColor("#6E88A4")
        canvas.drawText("MAX SESSION", rightGroupLeft, top + dp(104f), textPaint)
        textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        textPaint.textSize = dp(14f)
        textPaint.color = Color.parseColor("#00E985")
        canvas.drawText(String.format("%.1fg", peakAccel), rightGroupRight - dp(20f), top + dp(104f), textPaint)
    }
