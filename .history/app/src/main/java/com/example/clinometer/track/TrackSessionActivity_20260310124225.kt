...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private fun updateTopLeanTelemetry(leanAngle: Float = currentCalibratedLean) {
        ...
        val displayDeadband = if (isRecording) leanDisplayDeadbandDeg else leanDisplayDeadbandIdleDeg
        val displaySmoothing = if (isRecording) leanDisplaySmoothingAlpha else leanDisplaySmoothingAlphaIdle
        ...
        if (!hasDisplayLeanAngle) {
            ...
            displayLeanAngle += displaySmoothing * (targetLean - displayLeanAngle)
        }
        ...
        event?.let { ev ->
            ...
            when (ev.sensor.type) {
                ...
                Sensor.TYPE_GYROSCOPE -> {
                    ...
                    val rollRateSmoothing = if (isRecording) 0.25f else 0.55f
                    latestRollRateDegPerSec =
                        rollRateSmoothing * rollRateDeg + (1f - rollRateSmoothing) * latestRollRateDegPerSec
