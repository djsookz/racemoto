...
object DragCalibration {
    ...
    private fun loadFromPrefs() {
        ...
        prefs?.let { p ->
            ...
            if (isUniversalCalibrated) {
                ...
                // Заредено от prefs, но ВИНАГИ преизчисляваме от forward × gravity
                // за да сме независими от грешки в стари записани данни.
                val fwd = forwardVector
                val grv = gravityVector
                val rx = fwd[1]*grv[2] - fwd[2]*grv[1]
                val ry = fwd[2]*grv[0] - fwd[0]*grv[2]
                val rz = fwd[0]*grv[1] - fwd[1]*grv[0]
                val rMag = kotlin.math.sqrt((rx*rx + ry*ry + rz*rz).toDouble()).toFloat()
                rightVector = if (rMag > 0.01f) floatArrayOf(rx/rMag, ry/rMag, rz/rMag)
                              else floatArrayOf(
    ...
    fun lockPortraitAxes(forward: FloatArray, lateral: FloatArray, baseline: FloatArray, 
        ...
        // RIGHT = normalize(cross(FORWARD, GRAVITY_UP))
        // baseline от акселерометъра сочи "нагоре" (реакционна сила), не надолу.
        // RIGHT = cross(FORWARD, UP) — правилна посока надясно
        rightVector = floatArrayOf(
            forward[1] * baseline[2] - forward[2] * baseline[1],
            forward[2] * baseline[0] - forward[0] * baseline[2],
            forward[0] * baseline[1] - forward[1] * baseline[0]
        )
    ...
    fun lockUniversalCalibration(gravity: FloatArray, forward: FloatArray, maxVibration: Float) {
        ...
        // RIGHT = cross(FORWARD, GRAVITY_UP) — gravity от акселерометъра сочи UP
        // RIGHT = cross(FORWARD, UP) — правилна посока надясно
        ...
        rightVector = floatArrayOf(
            forwardVector[1] * downNorm[2] - forwardVector[2] * downNorm[1],
            forwardVector[2] * downNorm[0] - forwardVector[0] * downNorm[2],
            forwardVector[0] * downNorm[1] - forwardVector[1] * downNorm[0]
        )
