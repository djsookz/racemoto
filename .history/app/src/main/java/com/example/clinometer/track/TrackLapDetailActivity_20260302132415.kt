...
class TrackLapDetailActivity : AppCompatActivity() {
    ...
    private fun getSegmentAccelerationColor(startPoint: RoutePoint, endPoint: RoutePoint): Int {
        ...
        val accelThreshold = 0.22f
        val brakeThreshold = -0.28f
        val intensity = (abs(longitudinalMs2) / 2.8f).coerceIn(0f, 1f)

        return when {
            longitudinalMs2 >= accelThreshold -> {
                val r = (110f * (1f - intensity)).toInt().coerceIn(0, 255)
                val g = (170f + 85f * intensity).toInt().coerceIn(0, 255)
                val b = (70f * (1f - intensity)).toInt().coerceIn(0, 255)
                Color.rgb(r, g, b)
            }

            longitudinalMs2 <= brakeThreshold -> {
                val r = (200f + 55f * intensity).toInt().coerceIn(0, 255)
                val g = (95f * (1f - intensity)).toInt().coerceIn(0, 255)
                val b = (35f * (1f - intensity)).toInt().coerceIn(0, 255)
                Color.rgb(r, g, b)
            }
    ...
    private fun showFullRoute() {
        ...
        // Добавяме маркера върху маршрута
    ...
    private fun calculateGForceAtTime(timeInSeconds: Float): Float {
        // This would need to be calculated from the stored acceleration data
        // For now, return a sample value
        return (Math.random() * 2.5).toFloat()
    }
