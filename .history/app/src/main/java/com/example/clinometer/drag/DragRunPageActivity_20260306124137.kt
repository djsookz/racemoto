...
class DragRunPageActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val initialQuality = DragCalibration.getUniversalCalibrationQualityReport()
        Log.d(
            "DragRunPage",
            "📍 Profile ID: $profileId, Calibrated: ${DragCalibration.isCalibrated}, Portrait: ${DragCalibration.isPortraitCalibrated}, Landscape: ${DragCalibration.isLandscapeCalibrated}, Quality=${initialQuality.level}(${initialQuality.score})"
        )
            ...
            when {
                ...
                !gpsReady -> {
                    ...
                    if (!DragCalibration.isCalibrated) {
                        ...
                        val qualityText = "Q:${qualityReport.level} ${qualityReport.score}/100"
                        tvStatus.text = "$accuracyIndicator $baseText ($qualityText)"
                        Log.d(
                            "DragRunPage",
                            "📊 Ready! DragCal: ${DragCalibration.isCalibrated}, LinearCal: $linearAccelCal, AccMode: $accuracyMode, Quality=${qualityReport.level}(${qualityReport.score})"
                        )
                    }
                }
            }
    ...
    override fun onResume() {
        ...
        DragCalibration.setProfile(profileId)
        val quality = DragCalibration.getUniversalCalibrationQualityReport()
        Log.d(
            "DragRunPage",
            "🔄 onResume - Profile ID: $profileId, Calibrated: ${DragCalibration.isCalibrated}, Portrait: ${DragCalibration.isPortraitCalibrated}, Landscape: ${DragCalibration.isLandscapeCalibrated}, Quality=${quality.level}(${quality.score})"
        )
        updateReadyStatus()
    }
