    ...
    private fun loadSessionData() {
        ...
        if (isMotorcycleSession) {
            ...
            if (leanExtremes != null) {
                tvMaxCorneringLeft.text = String.format(java.util.Locale.getDefault(), "%.1f°", leanExtremes.first)
                tvMaxCorneringRight.text = String.format(java.util.Locale.getDefault(), "%.1f°", leanExtremes.second)
            } else {
                val fallbackLean = parseDisplayedNumeric(maxLeanDisplay) ?: 0f
                tvMaxCorneringLeft.text = String.format(java.util.Locale.getDefault(), "%.1f°", fallbackLean)
                tvMaxCorneringRight.text = String.format(java.util.Locale.getDefault(), "%.1f°", fallbackLean)
            }
        } else {
