...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private var pointToPointStaged: Boolean = false
    private var pointToPointStageSide: Double = 0.0
    private val pointToPointStagingSpeedKmh: Float = 1.5f
    private val pointToPointLaunchSpeedKmh: Float = 4.0f
    private val pointToPointStartLineArmingMeters: Double = 8.0
    companion object {
    ...
    private fun startSession() {
        ...
        pointToPointStaged = false
        pointToPointStageSide = 0.0
    ...
    private fun handlePointToPointStagingAndStart(location: Location, startLineA: TrackPoint, startLineB: TrackPoint) {
        ...
        if (!pointToPointStaged) {
            if (distanceToStartLine <= pointToPointStartLineArmingMeters && speedKmh <= pointToPointStagingSpeedKmh) {
                pointToPointStaged = true
                pointToPointStageSide = currentSide
                showToast("READY: колата е на старта")
                android.util.Log.d("TrackSessionActivity", "POINT_TO_POINT staged near start line")
            }
            return
        }

        if (speedKmh < pointToPointLaunchSpeedKmh) {
            return
        }
        ...
        val sideChanged = (pointToPointStageSide > 0.0 && currentSide < 0.0) ||
            (pointToPointStageSide < 0.0 && currentSide > 0.0)

        if (strictCrossed || sideChanged) {
            android.util.Log.d("TrackSessionActivity", "POINT_TO_POINT launch detected from staging")
            beginTimedSession(location)
        }
    }
    ...
    private fun showAwaitingStartDialog() {
        val message = if (currentTrackMode == TrackMode.POINT_TO_POINT) {
            "Спри колата точно на старт линията и потегли, за да започне измерването."
        } else {
            "За да започне измерването, премини през старт/финал линията."
        }
        dismissAwaitingStartDialog()
