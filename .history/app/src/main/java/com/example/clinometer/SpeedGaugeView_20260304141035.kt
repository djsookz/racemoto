...
class SpeedGaugeView @JvmOverloads constructor(
    ...
    private fun drawLeanAngleGauge(canvas: Canvas) {
        ...
        canvas.drawArc(gaugeRect, 220f, -80f, false, arcPaint)

        val normalized = ((leanAngle.coerceIn(-90f, 90f) + 90f) / 180f)
        val markerAngle = 180f - (normalized * 180f)
        ...
        canvas.drawCircle(markerX, markerY, dp(4f), fillPaint)
    }
    ...
    private fun drawCenterDisk(canvas: Canvas) {
        ...
        canvas.drawText("${currentLeanAbs}°", centerX, centerY - radius * 0.56f, textPaint)

        fillPaint.color = Color.parseColor("#9AA7BB")
        canvas.drawCircle(centerX, centerY - radius * 0.69f, dp(2.5f), fillPaint)

        textPaint.textSize = dp(23f)
        textPaint.color = Color.parseColor("#ECEFF5")
        canvas.drawText("${maxLeanLeft.roundToInt()}°", centerX - radius * 0.56f, centerY + radius * 0.12f, textPaint)
        canvas.drawText("${maxLeanRight.roundToInt()}°", centerX + radius * 0.56f, centerY + radius * 0.12f, textPaint)
