...
class TrackLapCompareActivity : AppCompatActivity() {
    ...
    private lateinit var llModeTabs: android.widget.LinearLayout
    ...
    private fun setupChart() {
        ...
        tvStatGapCurrent.text = if (gapSec >= 0f) String.format("-%+.3f s", gapSec).replace("+", "") else String.format("+%.3f s", -gapSec)
        ...
        val currentDataSet = LineDataSet(currentEntries, "Current").apply {
            ...
            minSpeedKmh = route.minOfOrNull { it.speed } ?: 0f
            setDrawValues(false)
