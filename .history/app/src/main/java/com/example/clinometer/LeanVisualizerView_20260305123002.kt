...
class LeanVisualizerView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val safeInset = maxOf(referenceArcPaint.strokeWidth, wheelGlowPaint.strokeWidth * 0.5f)
        ...
        val radiusBase = min(radiusX, radiusY)
        ...
        val rearDistX = radiusX * 0.26f
        val rearDistY = radiusY * 0.24f
        val frontDistX = radiusX * 0.90f
        val frontDistY = radiusY * 0.76f
        val rearX = centerX + sin(leanRad) * rearDistX
        val rearY = groundY - cos(leanRad) * rearDistY
        val frontX = centerX + sin(leanRad) * frontDistX
        val frontY = groundY - cos(leanRad) * frontDistY

        val rearRadius = radiusBase * 0.24f
        val frontRadius = radiusBase * 0.23f
        ...
        val riderHalfW = radiusBase * 0.08f
        val riderHalfH = radiusBase * 0.15f
        canvas.drawOval(
        ...
        canvas.drawCircle(helmetX, helmetY, radiusBase * 0.10f, riderStrokePaint)
    }
