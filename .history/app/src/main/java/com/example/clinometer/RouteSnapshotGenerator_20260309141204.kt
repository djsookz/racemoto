...
object RouteSnapshotGenerator {
    ...
    private val oneTimeRefreshRaces = Collections.synchronizedSet(mutableSetOf<Long>())
    ...
    /**
     * Returns true only once per process for each raceId.
     * Used to avoid regenerating the same snapshot on every bind.
     */
    fun markOneTimeRefreshRequested(raceId: Long): Boolean {
        return oneTimeRefreshRaces.add(raceId)
    }
