...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    private val forceMotorcycleUi = true
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (forceMotorcycleUi && !isMotorcycle) {
            Log.w(
                "TrackSessionActivity",
                "Force motorcycle UI is enabled; overriding resolved mode to motorcycle"
            )
            isMotorcycle = true
        }
    ...
    private fun updateTopLeanTelemetry(leanAngle: Float = currentCalibratedLean) {
        ...
        when {
            absLean < 2f -> {
                tvTopLeanDirection.text = "• CENTER"
            }
            leanAngle < 0f -> {
