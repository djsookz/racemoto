...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
        ...
        val progress01 = forcedProgress?.coerceIn(0f, 1f) ?: run {
            if (!isRecording || awaitingStart || lapStartTime <= 0L) {
                ...
                if (targetDistance <= 0f || lapDistanceAccum <= 0f) {
                    ...
                    val baseProgress = (lapDistanceAccum / targetDistance).coerceIn(0f, 0.998f)
                    val lineRemaining = currentDistanceToFinishLineMeters
                    val nearFinishProgress = if (lineRemaining.isFinite() && lineRemaining >= 0f && lineRemaining < 120f) {
                        (1f - (lineRemaining / 120f)).coerceIn(0f, 0.998f)
                    } else {
                        0f
                    }

                    max(baseProgress, nearFinishProgress)
                }
            }
        }
    ...
    private fun updateInertialForcesFromLinearAcceleration(deviceLinearAccel: FloatArray) {
        ...
        updateSessionGForceStatistics(correctedLatG, correctedLongG, confidence)
