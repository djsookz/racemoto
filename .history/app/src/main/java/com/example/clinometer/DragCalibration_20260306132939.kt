...
object DragCalibration {
    ...
    /**
     * Изчислява signed lateral acceleration (right/left) за конкретна ориентация.
     * Положителна стойност = надясно, отрицателна = наляво.
     */
    fun getSignedLateralAcceleration(rawAccel: FloatArray, isLandscape: Boolean): Float {
        if (!hasCalibrationFor(isLandscape)) return 0f
        ...
        val downNorm = floatArrayOf(
            baseline[0] / gravityMag,
            baseline[1] / gravityMag,
            baseline[2] / gravityMag
        )

        val right = floatArrayOf(
            downNorm[1] * forward[2] - downNorm[2] * forward[1],
            downNorm[2] * forward[0] - downNorm[0] * forward[2],
            downNorm[0] * forward[1] - downNorm[1] * forward[0]
        )
        ...
        val rightNorm = floatArrayOf(
            right[0] / rightMag,
            right[1] / rightMag,
            right[2] / rightMag
        )

        return linearAccel[0] * rightNorm[0] +
               linearAccel[1] * rightNorm[1] +
               linearAccel[2] * rightNorm[2]
    }
