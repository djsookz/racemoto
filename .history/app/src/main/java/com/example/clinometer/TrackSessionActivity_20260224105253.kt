...
import com.example.clinometer.track.catalog.CustomTrackDefinitionMapper
...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
    ...
    private fun loadTrackData() {
        ...
        if (isOfficial) {
            ...
            if (customTrack != null) {
                val trackDefinition = CustomTrackDefinitionMapper.fromCustomTrack(customTrack)
                trackName = trackDefinition.name
                currentTrackMode = trackDefinition.mode
                ...
                if (trackDefinition.mode == TrackMode.CIRCUIT && trackPoints.size >= 4) {
                ...
                android.util.Log.d("TrackSessionActivity", "Loaded custom track via TrackDefinition: ${trackDefinition.name} (${trackDefinition.mode}) with ${trackPoints.size} points")
    ...
    private fun markBestLapInUi(bestLapNumber: Int) {
        for (index in 0 until llLapsContainer.childCount) {
            ...
            tvLapNumber.text = if (lapNumber == bestLapNumber) "👑 $baseLabel" else baseLabel
        }
    }
