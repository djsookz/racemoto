...
class CustomTrackBuilderActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        etTrackName.addTextChangedListener {
            updateBuilderState()
        }
    ...
    private fun addSpecialPoint(geoPoint: GeoPoint, pointType: CustomTrack.TrackPoint.PointType) {
        when (pointType) {
            ...
            CustomTrack.TrackPoint.PointType.START -> {
                upsertSinglePoint(geoPoint, "Старт", R.drawable.ic_track_point_start)
            }
            CustomTrack.TrackPoint.PointType.FINISH -> {
                upsertSinglePoint(geoPoint, "Финиш", R.drawable.ic_track_point_finish)
            }
    ...
    private fun upsertSinglePoint(geoPoint: GeoPoint, title: String, iconRes: Int) {
        val existing = trackMarkers.firstOrNull { it.title == title }
        existing?.let {
            mapView.overlays.remove(it)
            trackMarkers.remove(it)
        }
        addSinglePoint(geoPoint, title, iconRes)
    }
    ...
    private fun saveTrack() {
        ...
        // Add start/finish line for circuit mode
    ...
    private fun validateTrack(): Boolean {
        when (trackType) {
            CustomTrack.TrackType.CIRCUIT -> {
                if (startFinishLinePoints.size != 2) {
                    Toast.makeText(this, "За обиколка добавете 2 старт/финиш точки за линия", Toast.LENGTH_SHORT).show()
                    return false
                }

                if (currentTrackPoints.size < 3) {
                    Toast.makeText(this, "За обиколка начертайте поне 3 точки", Toast.LENGTH_SHORT).show()
                    return false
                }
            }
            CustomTrack.TrackType.POINT_TO_POINT -> {
                val startCount = countMarkersByTitle("Старт")
                val finishCount = countMarkersByTitle("Финиш")

                if (startCount != 1 || finishCount != 1) {
                    Toast.makeText(this, "За точка-до-точка са нужни точно 1 старт и 1 финиш", Toast.LENGTH_SHORT).show()
                    return false
                }

                if (currentTrackPoints.size < 2) {
                    Toast.makeText(this, "За точка-до-точка начертайте поне 2 точки по трасето", Toast.LENGTH_SHORT).show()
                    return false
                }
            }
        }
    ...
    private fun countMarkersByTitle(title: String): Int {
        return trackMarkers.count { it.title == title }
    }
    ...
    private fun updateBuilderState() {
        ...
        val canSave = when (trackType) {
            CustomTrack.TrackType.CIRCUIT -> hasName && startFinishLinePoints.size == 2 && currentTrackPoints.size >= 3
            CustomTrack.TrackType.POINT_TO_POINT -> hasName && startCount == 1 && finishCount == 1 && currentTrackPoints.size >= 2
        }

        btnSave.isEnabled = canSave

        if (!isDrawing && !isRoutingInProgress) {
            tvStatus.text = when (trackType) {
                CustomTrack.TrackType.CIRCUIT -> {
                    when {
                        !hasName -> "Стъпка 1/3: Въведете име на трасето"
                        startFinishLinePoints.size < 2 -> "Стъпка 2/3: Добавете 2 точки за старт/финиш линия (${startFinishLinePoints.size}/2)"
                        currentTrackPoints.size < 3 -> "Стъпка 3/3: Начертайте трасето (поне 3 точки)"
                        else -> "✅ Трасето е готово за запазване"
                    }
                }
                CustomTrack.TrackType.POINT_TO_POINT -> {
                    when {
                        !hasName -> "Стъпка 1/4: Въведете име на трасето"
                        startCount == 0 -> "Стъпка 2/4: Добавете старт точка"
                        finishCount == 0 -> "Стъпка 3/4: Добавете финиш точка"
                        currentTrackPoints.size < 2 -> "Стъпка 4/4: Начертайте маршрута (поне 2 точки)"
                        else -> "✅ Трасето е готово за запазване"
                    }
                }
            }
