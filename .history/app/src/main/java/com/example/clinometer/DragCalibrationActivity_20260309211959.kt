...
class DragCalibrationActivity : AppCompatActivity(), SensorEventListener {
    ...
    // MOTO LEAN калибрация — открива forward axis от lean наляво
    private var isMotoProfile = false
    private var gyroscope: android.hardware.Sensor? = null
    private val leanGyroSamplesX = mutableListOf<Float>()
    private val leanGyroSamplesY = mutableListOf<Float>()
    private val leanGyroSamplesZ = mutableListOf<Float>()
    private var leanPhaseActive = false
    private val LEAN_SAMPLES_NEEDED = 100  // ~1sec @ 100Hz
    ...
    private fun startLearningLean() {
        ...
        leanGyroSamplesX.clear()
        leanGyroSamplesY.clear()
        leanGyroSamplesZ.clear()
        val statusView = if (calibratingOrientation == "portrait") tvPortraitStatus else tvLandscapeStatus
        statusView.text = "🏍️ Lean LEFT ~30° and hold"
        statusView.setTextColor(ContextCompat.getColor(this, android.R.color.holo_blue_light))
        // Регистрираме гироскопа за lean detection
    ...
    override fun onSensorChanged(event: SensorEvent) {
        when (event.sensor.type) {
            ...
            Sensor.TYPE_GYROSCOPE -> {
                // Мото lean phase — открива forward axis от lean наляво
                if (leanPhaseActive) {
                    detectLeanForward(event.values[0], event.values[1], event.values[2])
                }
            }
        }
    }

    private fun detectLeanForward(gx: Float, gy: Float, gz: Float) {
        val gyroMag = sqrt(gx * gx + gy * gy + gz * gz)
        // Threshold ~ 15°/s — достатъчен за lean, игнорира малките вибрации
        if (gyroMag > 0.26f) {
            leanGyroSamplesX.add(gx)
            leanGyroSamplesY.add(gy)
            leanGyroSamplesZ.add(gz)
            if (leanGyroSamplesX.size >= LEAN_SAMPLES_NEEDED) {
                completeLeanPhase()
            }
        }
    }

    private fun completeLeanPhase() {
        leanPhaseActive = false

        val avgX = leanGyroSamplesX.average().toFloat()
        val avgY = leanGyroSamplesY.average().toFloat()
        val avgZ = leanGyroSamplesZ.average().toFloat()

        // Намираме доминантната ос — тя е forward axis
        // Lean наляво = ПОЛОЖИТЕЛНО въртене около forward ос (right-hand rule)
        val absX = abs(avgX); val absY = abs(avgY); val absZ = abs(avgZ)
        val forwardAxis = when {
            absX >= absY && absX >= absZ -> floatArrayOf(if (avgX > 0) 1f else -1f, 0f, 0f)
            absY >= absX && absY >= absZ -> floatArrayOf(0f, if (avgY > 0) 1f else -1f, 0f)
            else                         -> floatArrayOf(0f, 0f, if (avgZ > 0) 1f else -1f)
        }

        Log.d("DragCalibration", "✅ Moto lean COMPLETE! ${leanGyroSamplesX.size} samples")
        Log.d("DragCalibration", "   Avg gyro: [${'$'}{"%.3f".format(avgX)}, ${'$'}{"%.3f".format(avgY)}, ${'$'}{"%.3f".format(avgZ)}] rad/s")
        Log.d("DragCalibration", "   Forward vector: [${forwardAxis[0]}, ${forwardAxis[1]}, ${forwardAxis[2]}]")
