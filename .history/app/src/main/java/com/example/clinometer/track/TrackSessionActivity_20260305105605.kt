...
class TrackSessionActivity : BaseActivity(), SensorEventListener, LocationListener {
        ...
        val progress01 = forcedProgress?.coerceIn(0f, 1f) ?: run {
            if (!isRecording || awaitingStart || lapStartTime <= 0L) {
                ...
                if (targetDistance <= 0f) {
                    ...
                    val baseProgress = (lapDistanceAccum / targetDistance).coerceIn(0f, 1f)
                    val distanceCap = progressCapByLineDistance(targetDistance)
                    baseProgress.coerceAtMost(distanceCap)
                }
            }
        }
    ...
    private fun progressCapByLineDistance(targetDistanceMeters: Float): Float {
        val distanceToLine = currentDistanceToLapLineMeters
        if (!distanceToLine.isFinite()) return 0.985f

        val nearWindowMeters = (targetDistanceMeters * 0.14f).coerceIn(120f, 700f)
        val farCap = 0.94f
        if (distanceToLine >= nearWindowMeters) return farCap

        val approachRatio = 1f - (distanceToLine / nearWindowMeters).coerceIn(0f, 1f)
        return (farCap + (0.985f - farCap) * approachRatio).coerceIn(farCap, 0.985f)
    }
    ...
    private fun updateDistanceToLapLine(location: Location) {
        ...
        val lineIndices = when {
            currentTrackMode == TrackMode.POINT_TO_POINT && !awaitingStart && trackPoints.size >= 4 -> {
                (trackPoints.size - 2) to (trackPoints.size - 1)
            }
            !awaitingStart && startFinishLineIndices.size >= 4 && trackPoints.size >= 4 -> {
                (trackPoints.size - 2) to (trackPoints.size - 1)
            }
            else -> 0 to 1
        }

        val lineStart = trackPoints[lineIndices.first]
        val lineEnd = trackPoints[lineIndices.second]
        currentDistanceToLapLineMeters = gateCrossingEngine.distanceToLineMeters(
