...
class TrackSessionDetailActivity : AppCompatActivity() {
    ...
    private fun loadComparableOutings(): List<ComparableOuting> {
        ...
        val currentTrackOnlyId = extractTrackIdFromSessionId(trackId)
        ...
        sessionIds.forEach { sessionId ->
            ...
            for (candidateOuting in 1..outingCount) {
                if (sessionId == trackId && candidateOuting == outingNumber) continue
    ...
    private fun openLapCompare(target: ComparableOuting) {
        val currentPrefs = getSharedPreferences("track_outings", MODE_PRIVATE)
        val currentLapDataCount = currentPrefs.getInt("${trackId}_outing_${outingNumber}_lap_data_count", 0)
        if (currentLapDataCount <= 0) {
            Toast.makeText(this, "Текущата сесия няма lap данни за сравнение", Toast.LENGTH_SHORT).show()
            return
        }

        val currentBestLap = resolveBestLapNumber(currentPrefs, trackId, outingNumber, currentLapDataCount)

        startActivity(Intent(this, TrackLapCompareActivity::class.java).apply {
            putExtra("current_session_id", trackId)
            putExtra("current_outing_number", outingNumber)
            putExtra("current_lap_number", currentBestLap)
            ...
            putExtra("track_name", tvTrackName.text?.toString().orEmpty())
        })
    }
    ...
    private fun openLapDetail(lapNumber: Int, lapTime: String) {
        ...
        if (parsedLapData == null || parsedLapData.routePoints.isEmpty()) {
            openLegacyLapDetail(lapNumber, lapTime)
            return
        }
        ...
        val intent = Intent(this, MapActivity::class.java).apply {
            ...
            putExtra(MapActivity.EXTRA_TRACK_CONTEXT, true)
            putExtra(MapActivity.EXTRA_TRACK_ID, trackIdForName)
            putExtra(MapActivity.EXTRA_TRACK_NAME, trackDisplayName)
            putExtra(MapActivity.EXTRA_TRACK_IS_MOTORCYCLE, isMotorcycle)
            putExtra(MapActivity.EXTRA_TRACK_SESSION_ID, trackId)
            putExtra(MapActivity.EXTRA_TRACK_LAP_NUMBER, lapNumber)
            putExtra(MapActivity.EXTRA_TRACK_OUTING_NUMBER, outingNumber)
            putExtra(MapActivity.EXTRA_TRACK_IS_POINT_TO_POINT, isPointToPointSession)
        }
        startActivity(intent)
        overridePendingTransition(0, 0)
    }

    private fun openLegacyLapDetail(lapNumber: Int, lapTime: String) {
    ...
    private fun loadSessionData() {
        ...
        if (isMotorcycleSession) {
            ...
            if (leanExtremes != null) {
                tvMaxCorneringLeft.text = formatLeanAngleCeil(leanExtremes.first)
                tvMaxCorneringRight.text = formatLeanAngleCeil(leanExtremes.second)
            } else {
                val fallbackLean = parseDisplayedNumeric(maxLeanDisplay) ?: 0f
                val fallbackLeanText = formatLeanAngleCeil(fallbackLean)
    ...
    private fun extractTrackIdFromSessionId(sessionId: String): String {
        ...
        if (officialMatch != null) {
            android.util.Log.d("TrackSessionDetailActivity", "extractTrackIdFromSessionId: matched official='$officialMatch'")
        ...
        if (withoutProfileId.startsWith("custom_")) {
            ...
            val resolvedCustom = customIdMatch ?: withoutProfileId
            android.util.Log.d("TrackSessionDetailActivity", "extractTrackIdFromSessionId: matched custom='$resolvedCustom'")
    ...
    private fun formatLeanAngleCeil(value: Float): String {
        val roundedUp = kotlin.math.ceil(value.toDouble()).toInt().coerceAtLeast(0)
        return "$roundedUp°"
    }
