...
class SpeedGaugeView @JvmOverloads constructor(
    ...
    private fun drawCenterDisk(canvas: Canvas) {
        ...
        if (isMotorcycle) {
            ...
            canvas.drawText("${maxLeanLeft.roundToInt()}°", centerX - radius * 0.66f, centerY + radius * 0.05f, textPaint)
            canvas.drawText("${maxLeanRight.roundToInt()}°", centerX + radius * 0.66f, centerY + radius * 0.05f, textPaint)
        }
    ...
    private fun drawMotoTelemetry(canvas: Canvas) {
        ...
        val metricsTopRatio = 0.90f
        val barOffsetDp = 24f
        val metricY = height * metricsTopRatio
        ...
        drawBottomMetric(
            ...
            valueText = String.format("%.1f G", accelG),
            title = "Acceleration"
        )

        val barsY = metricY + dp(barOffsetDp)
        drawCompactGBar(
            canvas = canvas,
            x = width * 0.17f,
            y = barsY,
    ...
    private fun drawCarTelemetry(canvas: Canvas) {
        ...
        val graphCenterY = height * 0.90f
        val graphRadius = dp(34f)
        ...
        canvas.drawCircle(dotX, dotY, dp(5f), fillPaint)
        ...
        canvas.drawText(String.format("%.2f G", totalG), graphCenterX, graphCenterY - graphRadius - dp(8f), textPaint)
    }
